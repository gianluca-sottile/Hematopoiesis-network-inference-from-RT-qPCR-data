...
# for(.id in c(5, 6, 7, 8)) {
  ...
  percorso <- paste0("results/02_nu_bic/")
  ...
  lambda1.min.ratio <- .1
  nlambda2 <- 1L                                   # number of lambda2-values
  lambda2.min.ratio <- 1L
  ...
  mse_B <- array(0, dim = c(nsim, K, nlambda1, nrho1, 1), 
                 dimnames = list(nsim = NULL, K = paste0("group", seq_len(K)),
                                 nlambda1 = 1:nlambda1, nrho1 = 1:nrho1, 
  ...
  prcurve_Tht <- array(0, dim = c(nsim, K, nlambda1, 10L, 2, 1), 
                       dimnames = list(nsim = NULL, K = paste0("group", seq_len(K)), 
                                       lambda1 = 1L:nlambda1, nrho1 = 1L:10L, 
  ...
  prcurve_data_Tht <- array(0, dim = c(nlambda1, 10L, K, 2, 1), 
                            dimnames = list(lambda1 = 1L:nlambda1, rho1 = 1L:10L, 
                                            K = paste0("group", seq_len(K)), 
                                            type = c("recall", "precision"), 
                                            estimator = c("jcglasso")))
  ...
  prcurve_Thtx <- array(0, dim = c(nsim, K, nlambda1, 10L, 2, 1), 
                        dimnames = list(nsim = NULL, K = paste0("group", seq_len(K)), 
                                        lambda1 = 1:nlambda1, nrho1 = 1L:10L, 
                                        type = c("recall", "precision"), 
                                        estimator = c("jcglasso")))
  ...
  prcurve_data_Thtx <- array(0, dim = c(nlambda1, 10L, K, 2, 1), 
                             dimnames = list(lambda1 = 1:nlambda1, rho1 = 1L:10L, 
                                             K = paste0("group", seq_len(K)), 
                                             type = c("recall", "precision"), 
                                             estimator = c("jcglasso")))
  ...
  prcurve_B <- array(0, dim = c(nsim, K, 10L, nrho1, 2, 1), 
                     dimnames = list(nsim = NULL, K = paste0("group", seq_len(K)), 
                                     lambda1 = 1L:10L, nrho1 = 1:nrho1, 
                                     type = c("recall", "precision"), 
                                     estimator = c("jcglasso")))
  ...
  prcurve_data_B <- array(0, dim = c(10L, nrho1, K, 2, 1), 
                          dimnames = list(lambda1 = 1L:10L, rho1 = 1:nrho1, 
                                          K = paste0("group", seq_len(K)), 
                                          type = c("recall", "precision"), 
                                          estimator = c("jcglasso")))
  ...
  Xorig <- X <- Theta.x <- Sigma.x <- Theta.y <- Sigma.y <- B <- mu <- E <- Y <- S <- Z <- Z2 <- vector(mode = "list", length = K)
  ...
  for(k in seq_len(K)) {
    ...
    sim <- huge.generator(n = n, d = q[i.q], graph = "hub", prob = .95) # random # hub
  ...
  out0 <- jcglasso(data = Z2, nrho = 25L, thr.em = .01, 
                   trace = 0L, penalty = "group", alpha = alpha)
  plot(BIC(out0))
  outQfun0 <- QFun2(out0, mle = TRUE)
  plot(BIC(out0, Qfun = outQfun0))
  plot(select.jcglasso(out0, BIC(out0, Qfun = outQfun0)))
  rho.x <- select.jcglasso(out0, BIC(out0, Qfun = outQfun0))$rho / alpha
  ...
  for(h in 1:(nruns)) {
    for(j in 1:(subnsim)) {
      ...
      out <- jcglasso(data = Z, nrho = 1L, nlambda = 1L, thr.bcd = 1L, thr.em = 1L, 
                      trace = 0L, penalty = "group", alpha = alpha, rho.x = rho.x)
      rho1 <- max(out$rho) * c2 / out$alpha
      # rho2 <- rho1 * out$alpha
      lambda1 <- max(out$lambda) * c2 / out$alpha
      # lambda2 <- lambda1 * out$alpha
      out <- jcglasso(data = Z, rho = rho1, lambda = lambda1, trace = 0L, 
                      penalty = "group", alpha = alpha, thr.em = 1e-2, rho.x = rho.x)
  ...
  for(i in seq_len(nlambda1)){
    mse_tbl_Tht["mean", "jcglasso", i, ] <- colMeans(sapply(seq_len(K), function(k) apply(mse_Tht[, k, i, , "jcglasso"], 1L, min)))
    mse_tbl_Tht["sd", "jcglasso", i, ] <- apply(sapply(seq_len(K), function(k) apply(mse_Tht[, k, i, , "jcglasso"], 1L, min)), 2L, sd)
    
    mse_tbl_Thtx["mean", "jcglasso", i, ] <- colMeans(sapply(seq_len(K), function(k) apply(mse_Thtx[, k, i, , "jcglasso"], 1L, min)))
    mse_tbl_Thtx["sd", "jcglasso", i, ] <- apply(sapply(seq_len(K), function(k) apply(mse_Thtx[, k, i, , "jcglasso"], 1L, min)), 2L, sd)
  }
  ...
  for(o in seq_len(nrho1)){
    mse_tbl_B["mean", "jcglasso", o, ] <- colMeans(sapply(seq_len(K), function(k) apply(mse_B[, k, , o, "jcglasso"], 1L, min)))
    mse_tbl_B["sd", "jcglasso", o, ] <- apply(sapply(seq_len(K), function(k) apply(mse_B[, k, , o, "jcglasso"], 1L, min)), 2, sd)
  }
  ...
  for(i in seq_len(nlambda1)){
    prcurve_data_Tht[i, , , "recall", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_Tht[, k, i, , "recall", "jcglasso"], 2, mean, na.rm=TRUE))
    prcurve_data_Tht[i, , , "precision", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_Tht[, k, i, , "precision", "jcglasso"], 2, mean, na.rm=TRUE))
    
    prcurve_data_Thtx[i, , , "recall", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_Thtx[, k, i, , "recall", "jcglasso"], 2, mean, na.rm=TRUE))
    prcurve_data_Thtx[i, , , "precision", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_Thtx[, k, i, , "precision", "jcglasso"], 2, mean, na.rm=TRUE))
  }
  ...
  for(o in seq_len(nrho1)){
    prcurve_data_B[, o, , "recall", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_B[, k, , o, "recall", "jcglasso"], 2, mean, na.rm=TRUE))
    prcurve_data_B[, o, , "precision", "jcglasso"] <- sapply(seq_len(K), function(k) apply(prcurve_B[, k, , o, "precision", "jcglasso"], 2, mean, na.rm=TRUE))
  }
