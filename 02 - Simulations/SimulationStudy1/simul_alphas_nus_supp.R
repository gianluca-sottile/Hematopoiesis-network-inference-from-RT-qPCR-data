...
for(.nu in .nu.vec) {
  ...
  for(k in 1:K) {
    tempTheta <- simulTheta(n, p, prob = .005, alpha = unconnectedTheta[[k]], seed = 1234, #prob = c(0.005, 0.02)
                            vis = FALSE, graph = "random", rewire = .5, class = 1)
  ...
  # pdf("~/Downloads/structureK_Example_100324.pdf", width = 16*.75, height = 12*.75)
  par(mfrow = c(2, 3))
  for(k in seq_len(K)) {
    ...
    axis(1, at = c(0, .2, .4, .6, .8, 1), labels = c(0, 10, 20, 30, 40, 50)*4, cex.axis = 2)
    axis(2, at = c(0, .2, .4, .6, .8, 1), labels = c(0, 10, 20, 30, 40, 50)*4, cex.axis = 2)
    image(Theta.x[[k]] != 0, col = c("white", "black"), 
          ylab = "", cex.lab = 2, cex.main = 3, 
          main = ifelse(k == 1, expression(Omega), ""), axes = FALSE)
    axis(1, at = c(0, .2, .4, .6, .8, 1), labels = c(0, 5, 10, 15, 20, 25)*2, cex.axis = 2)
    axis(2, at = c(0, .2, .4, .6, .8, 1), labels = c(0, 5, 10, 15, 20, 25)*2, cex.axis = 2)
  ...
  for(i in 1:n.sim) {
    ...
    for(a in 1:length(alpha.seq)) {
      ...
      for(r in 1:length(c2)) {
        ...
        for(k in seq_len(K)) {
          out_prcurve <- PrecRecAUC(A = tmp2.2$B[, , k, , 1L], B = B[[k]], unpenalized = (up!=0))
    ...
    # if(i %% 5 == 0) save.image(paste0("~/Downloads/Simul1.2_13032024_n", n, "p", p, "q", q,
...
for(.nu in .nu.vec) {
  ...
  # load(paste0("~/Downloads/Simul1.2_13032024_n", n, "p", p, "q", q,
  #             "NA", perc.NA*100, "Xna", perc.X.na*100, "Yna", perc.Y.na*100,
  #             "alpha", alpha.true, "nu", .nu, ".RData"))
  # load(paste0("~/Downloads/Simul1.3_13032024_n", n, "p", p, "q", q,
  #             "NA", perc.NA*100, "Xna", perc.X.na*100, "Yna", perc.Y.na*100,
  #             "alpha", alpha.true, "nu", .nu, ".RData"))
...
ggsave("~/Downloads/mse_curves_130324_2.pdf", plot = pall, device = "pdf", units = "in",
...
ggsave("~/Downloads/auc_curves_130324_2.pdf", plot = p3, device = "pdf", units = "in",
       width = 12, height = 16, scale = .75)
