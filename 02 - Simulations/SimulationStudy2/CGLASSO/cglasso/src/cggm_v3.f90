...
!           '2' glassosub
!           '3' cglasso_v1
subroutine cggm_v3(n,p,Y,Id,nP,InfoP,lo,up,ym,yv,pendiag,wTht,ntp,rho,maxit_em,thr_em,maxit_bcd,thr_bcd,Yipt,B,mu,R,S,&
...
B_n(p),Sgm_o(p,p),Sgm_n(p,p),Tht_n(p,p),rho_gl(p,p),dB,dSgm,Tht_o(p,p)
...
!            where (wTht.eq.big) wTht = 0.0d0
        ...
        call glassosub_v2(p,S_n,pendiag,rho_gl,maxit_bcd,thr_bcd,Sgm_n,Tht_n,ncomp_n,Ck_n,pk_n,nnit,conv,trace)
        if(conv.ne.0) then
            subrout = 3
!            where (wTht.eq.big) wTht = 0.0d0
            return
