...
jmmle.1step = function(Y.list, # K-length list of Y-matrices, each n X q
    ...
    iter = 0; CONVERGE=FALSE; CONVERGE1=FALSE; #refit.B=FALSE;
    ...
    while(!CONVERGE){
        ...
        Normdiff[iter] = sqrt(sum(B_new.array - B_old.array)^2)/(.1 + sqrt(sum(B_new.array^2))) +
            sqrt(sum(Theta_new.array - Theta_old.array)^2)/(.1 + sqrt(sum(Theta_new.array^2)))
        # if (iter == 1){
