...
for(i in 1:n.sim) {
  ...
  # plot(sqrt(log(q)/n) * exp(seq(log(2), log(.1), l = 5)), rowSums(tmpMSE), type = "b")
  ...
  # if(i %% 5 == 0) save.image(paste0("~/Downloads/Simul2.1_13032024_n", n, "p", p, "q", q,
...
MSE1 <- sapply(1:K, \(k) apply(MSE_JMMLE[, , k], 2, mean))# * c(1000/(p*(q+1)), 1000/(.5*p*(p+1)), 1000/(.5*q*(q+1)))
MSE2 <- sapply(1:K, \(k) apply(MSE_CGLASSO[, , k], 2, mean))# * c(1000/(p*(q+1)), 1000/(.5*p*(p+1)), 1000/(.5*q*(q+1)))

MSE3 <- lapply(1:length(alpha.seq), \(a) 
               sapply(1:K, \(k) apply(MSE_JCGLASSO[a, , , k], 2, mean)))# * c(1000/(p*(q+1)), 1000/(.5*p*(p+1)), 1000/(.5*q*(q+1))))
