...
PrecRecAUC <- function(A, B, plot.it = FALSE) {
  n <- dim(A)[3L]
  tmp <- sapply(1:n, \(i) {
    data <- factor(1*c(A[, , i] != 0), levels = c(1,0))
    reference <- factor(1 * c(B != 0), levels = c(1,0))
    confusionMatrix(data = data, reference = reference, mode = "prec_recall")$byClass[c("Precision", "Recall")]
  })
  tmp <- cbind(c(1, sum(ncol(B) != 0) / sum(B != 0)), 
               tmp, 
               c(sum(B != 0) / prod(dim(B)), 1))
  ...
  for(i in 1:n.sim) {
    ...
    for(a in 1:length(alpha.seq)) {
      ...
      for(r in 1:length(c2)) {
        ...
        MSE[match(.nu, .nu.vec), a, i, "B", r, , seq_len(K)] <- sapply(seq_len(K), \(k) apply(tmp2.2$B[, , k, , 1L], 3, function(Bh) sum((Bh - B[[k]])^2) / sum(B[[k]] != 0)))
      ...
      for(l in 1:length(c2)) {
        ...
        MSE[match(.nu, .nu.vec), a, i, "Theta", l, , seq_len(K)] <- sapply(seq_len(K), \(k) apply(tmp3.2$Tht[1:p, 1:p, k, 1L, ], 3L, function(Th) sum((Th[U] - Theta.y[[k]][U])^2) / sum(Theta.y[[k]][U] != 0)))
      ...
      # if(i %% 5 == 0) save.image(paste0("~/Downloads/simul_new/SimulJRSSC_27102023_n", n, "p", p, "q", q, 
...
for(.nu in .nu.vec) {
  ...
  # load(paste0("~/Downloads/simul_new/SimulJRSSC_27102023_n", n, "p", p, "q", q,
  #             "NA", perc.NA*100, "Xna", perc.X.na*100, "Yna", perc.Y.na*100,
  #             "alpha", alpha.true, "nu", .nu, ".RData"))
...
# ggsave("~/Downloads/simul_new/mse_curves_271023.pdf", plot = pall, device = "pdf", units = "in", 
...
# ggsave("~/Downloads/simul_new/auc_curves_271023.pdf", plot = p3, device = "pdf", units = "in", 
#        width = 12, height = 16, scale = .75)
